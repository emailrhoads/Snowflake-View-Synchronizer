CREATE OR REPLACE VIEW "RESOURCE_CAPACITY"."PNMAC"."VW_MISSINGTIMESHEETENTRIESTHUNTER" AS 
SELECT
    ...
    ,hier.management_hierarchy as management_hierarchy
...
(select 
    ...
    ,ifnull(sum(hours.billable_hours),0) hours
    ,(CASE WHEN ifnull(sum(hours.billable_hours),0) >= 40 THEN '40+ Hours Entered' ELSE '<40 Hours Entered' END) as time_reporting_status
from 
    (select distinct lower(trim(vwfaci.employee_network_login)) as employee_network_login from "RESOURCE_CAPACITY"."PNMAC"."MVW_FORECASTANDACTUALCOMBINEDINFO" vwfaci where vwfaci.employee_id <> 'Forecast') as vw
LEFT JOIN
...
group by 
    ...
    LEFT JOIN "DW_ORG"."PNMAC"."VW_EMPLOYEE_HIERARCHY" hier ON employee_detail.employeeid = hier.employee_id
    WHERE year_week_count_by_id.week_start_date between ifnull(employee_detail.hiredate,'1900-01-01') and ifnull(employee_detail.terminationdate,current_timestamp)
        AND lower(hier.management_hierarchy) LIKE '%tracie hunter%';